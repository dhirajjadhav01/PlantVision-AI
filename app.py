...
with st.sidebar:
    ...
    if st.button("⌁   Disease Detection", use_container_width=True, key="pv_sidebar_detect_v2"):
        st.session_state.page = "Disease Detection"

    if st.button("⌂   Dashboard", use_container_width=True, key="pv_sidebar_dash_v2"):
        st.session_state.page = "Dashboard"

    if st.button("◈   Model Performance", use_container_width=True, key="pv_sidebar_perf_v2"):
...
# ============================================================
    ...
    <div class="info" style="margin: 14px 0 18px 0;">
        ...
        <p class="small" style="margin:0;">
            ...
            confidence, class probabilities, and an agricultural decision-support summary.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if image_source is not None:
        ...
        if analyze:
            ...
            # Agricultural decision support / model interpretation
            ...
            if confidence >= 0.90:
                ...
                confidence_note = "The model shows strong preference for the detected class."
            elif confidence >= 0.70:
                confidence_status = "Moderate-confidence prediction"
                confidence_note = "The result is useful for screening, but the leaf should be checked carefully."
            else:
                confidence_status = "Low-confidence prediction"
                confidence_note = "The image may be unclear or visually similar to another class. Consider capturing a clearer leaf image."

            s1, s2, s3 = st.columns(3)
            with s1:
                st.metric("Confidence status", confidence_status)
            with s2:
                st.metric("Runner-up", display_names[runner_up_class])
            with s3:
                st.metric("Confidence gap", f"{confidence_gap*100:.2f}%")
            ...
            st.markdown(f"""
            <div class="card">
                <p><b>Predicted class:</b> {display_names[predicted_class]}</p>
                <p><b>Prediction probability:</b> {confidence*100:.2f}%</p>
                <p><b>Second most likely class:</b> {display_names[runner_up_class]} ({runner_up_confidence*100:.2f}%)</p>
                <p><b>Decision margin:</b> {confidence_gap*100:.2f} percentage points</p>
                <p class="small">The decision is based on the probability distribution produced by the trained MobileNetV2 classifier.</p>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("#### 📚 Disease Knowledge")
            st.markdown(f"""
            <div class="card">
                <h3 style="margin-top:0;">{info["icon"]} {display_names[predicted_class]}</h3>
                <p><b>What it means:</b> {info["description"]}</p>
                <p><b>Recommended action:</b> {info["recommendation"]}</p>
                <p class="small"><b>Academic note:</b> PlantVision AI is an image-classification screening system. A final agricultural diagnosis should be confirmed using field observations or an agricultural expert.</p>
            </div>
            """, unsafe_allow_html=True)
